#ifndef     RANDOM_NUMBER_GERATION
#define     RANDOM_NUMBER_GERATION 1
...
namespace random
{
     ...
     */
    template <typename T>
    T weighted_number(T __mean, double __std_dev = 10.0)
    {
        static std::mt19937 gen(std::chrono::high_resolution_clock::now().time_since_epoch().count());
        static_assert(std::is_arithmetic<T>::value, "must be an arithmetic type");
     ...
     */
    template <typename T>
    T weighted_number(T __min, T __max, double __mean, double __std_dev = 10.0)
    {
        static std::mt19937 gen(std::chrono::high_resolution_clock::now().time_since_epoch().count());
        static_assert(std::is_arithmetic<T>::value, "must be an arithmetic type");
        std::normal_distribution<double> dist(__mean, __std_dev);
     ...
     */
    template <typename T>
    size_t weighted_index(std::vector<T> __v, size_t __mean, size_t __std_dev = 10.0)
    {
        static std::mt19937 gen(std::chrono::high_resolution_clock::now().time_since_epoch().count());
        std::normal_distribution<double> dist(__mean, __std_dev);
        size_t v;
